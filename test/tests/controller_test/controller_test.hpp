...
namespace deserializer_and_serializer_tests {
class BinaryConverter {
public:
  static std::vector<char> serializeData(const std::string &data, size_t size_of_data) {
    std::vector<char> result { }; // Ты такая крутышка!
...
private:   
  ...
  void generateBinaryJSONAndFiles() {
    ...
    for (size_t i { }; i < generated_files.size(); ++i) {
      auto serialized_data { BinaryConverter::serializeData(
        generated_files[i].second.data(), generated_files[i].second.size()) };
...
public:
  std::vector<char> generateSerializedData(size_t size_of_json, size_t number_of_files, size_t size_of_file) {
    generateJSON(size_of_json);
    generateFiles(number_of_files, size_of_file);
    generateBinaryJSONAndFiles();
    return generated_binary_data;
  }

  const Files &getGeneratedFiles() const noexcept {
...
public:
  void givenExpectedDataAndSerializedData(const Files &expected_data, const std::vector<char> &serialized_data) {
...
TEST_F(DeserializerTest, DeserializeCorrectSerializeData) {
  givenExpectedDataAndSerializedData(generator.getGeneratedFiles(), generator.generateSerializedData(100, 100, 100));
